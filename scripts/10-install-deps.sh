...
# Ubuntu 24.04 replaced the old wxWidgets 3.0 development package with 3.2.
# Permit an explicit override for other Debian-family distributions.
if [ -z "${WX_PACKAGE:-}" ]; then
  if apt-cache show libwxgtk3.2-dev >/dev/null 2>&1; then
    ...
    die "no supported wxWidgets development package found; run apt update or set WX_PACKAGE"
  fi
fi
...
$WX_PACKAGE libxml2 libxml2-utils lzop pngcrush
